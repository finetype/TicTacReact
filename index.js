(function(){
  ...
  function isWinner(game) {
    for (var i = 0; i < 3; i++) {
      ...
      if (col === -3 || col === 3) {
        alert("Winner is " + (col === 1 ? "X" : "O") + " on col " + (i+1) );
        return true;
      }
      ...
      if (row === -3 || row === 3) {
        alert("Winner is " + (row === 1 ? "X" : "O") + " on row " + (i+1) );
        return true;
      }
    }
    ...
    if (diag1 === -3 || diag1 === 3) {
      alert("Winner is " + (diag1 === 1 ? "X" : "O") + " on diagonal from top left ");
      return true;
    }

    var diag2 = [ game[0][2], game[1][1], game[2][0] ].reduce((a,b) => a + b);
    if (diag2 === -3 || diag2 === 3) {
      alert("Winner is " + (diag2 === 1 ? "X" : "O") + " on diagonal from top right");
      return true;
    }
